"""Implementation of model-agnostic meta-learning for flowers and fungi."""
...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):   # pylint: disable=unused-argument
        ...
        accuracy = utils.score(logits, labels)
        accuracies.append(accuracy)

        for _ in range(self._num_inner_steps):
            loss = F.cross_entropy(logits, labels)
            grads = autograd.grad(loss, inputs=list(parameters.values()), create_graph=train)
            new_params = {k: parameters[k] - self._inner_lrs[k] * grads[i] for i, k in enumerate(
                parameters.keys())}
    ...
    def _outer_step(self, task_batch, train):  # pylint: disable=unused-argument
        ...
        for task in task_batch:
            ...
            adapted_params, accuracies = self._inner_loop(images_support, labels_support, train)
