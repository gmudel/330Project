"""
Semi-supervised Protonet. Each task has N classes of labeled flowers and N classes of unlabeled
fungi. Use unlabeled fungi to adjust protonet center obtained using only flowers.
"""
import argparse
import os
...
class ProtoNet:
    ...
    def __init__(self, input_len, learning_rate, log_dir, model_num):
        ...
        self._network1 = model_list[model_num - 1](input_len, DEVICE)
        self._network2 = model_list[model_num - 1](input_len, DEVICE)
        self._optimizer = torch.optim.Adam(
            list(self._network1.parameters()) + list(self._network2.parameters()),
            lr=learning_rate
        )
    ...
    def _step(self, labeled_task_batch, unlabeled_task_batch=None):
        """Computes ProtoNet mean loss (and accuracy) on a batch of tasks.

        Args:
            labeled_task_batch (tuple[Tensor, Tensor, Tensor, Tensor]):
                batch of labeled tasks from a DataLoader
        ...
        if not unlabeled_task_batch:
            # don't use unlabeled tasks for eval
            for task in labeled_task_batch:
                ...
                features_support = self._network2(images_support)    # N*K, d
                features_query = self._network2(images_query)        # N*Q, d
            ...
            for labeled, unlabeled in zip(labeled_task_batch, unlabeled_task_batch):
                images_support, labels_support, images_query, labels_query = labeled
                unlabeled_images_support, _, _, _ = unlabeled

                unlabeled_images_support = unlabeled_images_support.to(DEVICE)
                images_support = images_support.to(DEVICE)
                labels_support = labels_support.to(DEVICE)
                images_query = images_query.to(DEVICE)
                labels_query = labels_query.to(DEVICE)

                features_support = self._network1(images_support)    # N*K, d
                features_query = self._network1(images_query)        # N*Q, d
                N = torch.max(labels_support) + 1
                K = features_support.shape[0] // N

                features_unlabeled_support = self._network2(unlabeled_images_support)    # N*M, d
                # prototypes of labeled data points
                prototypes = torch.mean(features_support.view(N, K, -1), dim=1)    # N, d

                # adjust prototypes based on clustering unlabeled data points
                logits_unlabeld = -(torch.cdist(features_unlabeled_support, prototypes) ** 2) # N*M, N
                probs_unlabeled = F.softmax(logits_unlabeld, dim=1)                # N*M, N
                weighted = probs_unlabeled.T @ features_unlabeled_support          # N, d
                weighted_counts = torch.sum(probs_unlabeled, dim=0) + K            # N,
                new_prototypes = (torch.sum(features_support.view(N, K, -1), dim=1) + weighted) / \
                                 weighted_counts.view(N, 1)                        # N, d

                logits_query = -(torch.cdist(features_query, new_prototypes) ** 2)     # N*Q, N
                loss = F.cross_entropy(logits_query, labels_query)
                loss_batch.append(loss)
                ...
                logits_support = -(torch.cdist(features_support, new_prototypes) ** 2)  # N*K, N
                accuracy_support = utils.score(logits_support, labels_support)
                accuracy_support_batch.append(accuracy_support)
    ...
    def train(self, dataloader_train, dataloader_unlabeled_train, dataloader_val, writer):
        """Train the ProtoNet.
        ...
        Args:
            ...
            dataloader_unlabeled_train (Dataloader): loader for unlabeled train tasks
            dataloader_val (DataLoader): loader for validation tasks
            writer (SummaryWriter): TensorBoard logger
        """
        print(f'Starting training at iteration {self._start_train_step}.')
        for i_step, (labeled_task_batch, unlabeled_task_batch) in enumerate(
                zip(dataloader_train, dataloader_unlabeled_train),
                start=self._start_train_step
        ):
            self._optimizer.zero_grad()
            loss, accuracy_support, accuracy_query = self._step(labeled_task_batch, unlabeled_task_batch)
            loss.backward()
...
def main(args):
    ...
    if not args.test:
        ...
        print(
            ...
            f'num_unlabeled_support={args.num_unlabeled_support}, '
            f'num_query={args.num_query}'
        )
        # for labeled portion (flowers) of a task
        ...
        # same N, but no Q
        dataloader_unlabeled_train = Fungi.get_fungi_dataloader(
            'train',
            args.batch_size,
            args.num_way,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_unlabeled_support', type=int, default=5,
                        help='number of unlabeled support examples per class in a task')
